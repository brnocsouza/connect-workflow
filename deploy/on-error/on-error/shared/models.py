...
class BaseImport:
    ...
    def _save(self, id_query: Union[None, int] = None):
        ...
        dict_data = {
            ...
            if not key.startswith('_') and key != 'id' and getattr(self, key) is not None
        }
        ...
        if id_query and id_query is not None:
            keys = dict_data.keys()
            update_replaces = ", ".join([f"`{key.strip()}`=%s" for key in keys])
            ...
            columns = ", ".join([f"`{item}`" for item in dict_data])
...
class Data(BaseImport):
    ...
    async def save_retry(self):
        ...
        self.retry_when = datetime.datetime.now() + datetime.timedelta(hours=2)
    ...
    async def find(self):
        ...
        store_data = await shared.find(query, (self.id,))
